...
import { homePathForSignupRole, normalizeSignupRole, pickLandingPortal } from '@/lib/portal-routing';
import { notifySignup } from '@/lib/auth/notify-signup';
import { safeInternalPath } from '@/lib/safe-redirect';
import { QR_SLUG_COOKIE, logScanEvent } from '@/lib/qr-attribution';

export type ConfirmOtpParams = {
  token_hash: string | null;
  type: 'email' | 'recovery' | 'invite' | null;
  role: string | null;
  next: string | null;
};

/**
 * Shared OTP-confirmation flow used by both the query-string route
 * (`/auth/confirm?token_hash=…&type=…`, the target of Supabase-generated signup
 * and invite links) and the path route (`/auth/confirm/<type>/<token_hash>`,
 * the target of our own password-reset email).
 *
 * The path form exists because a `token_hash=<hex>` query string is corrupted in
 * email transit: `=` immediately followed by two hex digits is a valid
 * quoted-printable escape, so a mail hop can swallow the separator into a single
 * (often non-printable) byte, and the server then never receives token_hash.
 * Carrying the values as path segments removes every `=` and `&`, so there is
 * nothing for quoted-printable to misread.
 */
export async function confirmOtp(request: NextRequest, params: ConfirmOtpParams) {
  const { origin } = new URL(request.url);
  const { token_hash, type } = params;
  const role = normalizeSignupRole(params.role);
