...
export async function POST(req: Request) {
  ...
  try {
    // Backstop for the auto-verify hold: pages sweep the rows they render, this
    // catches a certificate nobody has looked at since it was uploaded, so its
    // operator still gets told it cleared.
    const autoVerified = (await sweepInsuranceAutoVerify()).length;
    ...
    await logJobRun({ jobName: 'insurance_expiry', status: 'ok', durationMs: Date.now() - start, detail: { expired, reminded, autoVerified } });
    return NextResponse.json({ expired, reminded, autoVerified });
  } catch (e: any) {
