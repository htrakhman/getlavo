...
export default async function OperatorOverview({ searchParams }: { searchParams: { stripe_error?: string; stripe_msg?: string } }) {
  ...
  const [{ data: buildings }, { data: upcoming }, { data: payouts }, { data: pendingRows }] = await Promise.all([
    admin.from('partnerships')
      ...
      .eq('status', 'active'),
    // Declined days are excluded in the query, not after it, so the limit still
    // yields 5 real wash days. Matches /operator/wash-days, which files declined
    // days under their own section rather than Upcoming.
    sb.from('wash_days')
